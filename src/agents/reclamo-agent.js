...
export async function reclamoAgent(formCompilato, bollettaJson) {
  ...
  try {
    if (typeof window === 'undefined') {
      ...
      const r = spawnSync(CLAUDE_BIN, ['-p', userMsg, '--system-prompt', promptReclamo, '--model', MODEL], { encoding: 'utf8', timeout: 60_000 })
      if (r.error) throw new Error(r.error.message)
      if (r.status !== 0) throw new Error(r.stderr)
      return r.stdout.trim()
    }
    return callClaudeBrowser(promptReclamo, userMsg)
  } catch (err) {
    if (typeof window !== 'undefined') throw new Error('Non riesco a generare il reclamo. Riprova.')
    throw err
  }
}
...
async function testReclamoAgent() {
  ...
  let result
  try { result = await reclamoAgent(formCompilato, bollettaJson) } catch (e) { console.error('❌', e.message); process.exit(1) }
  console.log('\n✅  Testo reclamo:\n' + result)
...
import('url').then(({ fileURLToPath }) => {
  if (typeof process !== 'undefined' && process.argv?.[1] === fileURLToPath(import.meta.url)) testReclamoAgent()
}).catch(() => {})
