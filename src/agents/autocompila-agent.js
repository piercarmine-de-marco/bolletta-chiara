  ...
  try {
    if (typeof window === 'undefined') {
      ...
      const result = spawnSync(
        CLAUDE_BIN,
        ['-p', userMsg, '--system-prompt', promptAutocompila, '--model', MODEL],
    ...
    if (typeof window !== 'undefined') {
      throw new Error('Non riesco a compilare il modulo. Riprova.')
    }
    throw err
  }
}
...
async function testAutocompilaAgent() {
  ...
  let result
  try {
    result = await autocompilaAgent(bollettaJson)
  } catch (e) {
  ...
  const extra = Object.keys(result).filter(c => !campiAttesi.includes(c))
  if (mancanti.length) { console.warn('⚠️   Campi mancanti:', mancanti.join(', ')); process.exit(1) }
  if (extra.length) { console.warn('⚠️   Campi extra:', extra.join(', ')); process.exit(1) }
